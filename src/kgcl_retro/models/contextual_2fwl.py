...
class ContextualFGKGCL2FWL(nn.Module):
    ...
    def _apply_pair_feedback(
        ...
        src_atom = pairs[:, 0].to(device=atom_states.device)
        valid = src_atom > 0
        if valid.any():
            src_valid = src_atom[valid]
            feedback.index_add_(0, src_valid, projected[valid])
            ones = torch.ones((int(src_valid.numel()), 1), dtype=atom_states.dtype, device=atom_states.device)
            counts.index_add_(0, src_valid, ones)
    ...
    def _apply_pair_feedback_reference(
        self,
        atom_states: torch.Tensor,
        pair_states: torch.Tensor,
        pair_rel: torch.Tensor,
    ...
    def _score_actions(
        ...
        for mol_idx, (atom_start, atom_count) in enumerate(sparse.atom_scope):
            ...
            if candidate_pairs.numel():
                reverse_pairs = torch.stack([candidate_pairs[:, 1], candidate_pairs[:, 0]], dim=1)
                q_ij_idx, q_ij_valid = SparsePairLayerVectorized._lookup_pair_indices(
                    sparse.dec_carrier_pairs_base,
                    candidate_pairs,
                )
                q_ji_idx, q_ji_valid = SparsePairLayerVectorized._lookup_pair_indices(
                    sparse.dec_carrier_pairs_base,
                    reverse_pairs,
                )
                pair_valid = (q_ij_valid & q_ji_valid).unsqueeze(1)
                q_ij = dec_pair_states[q_ij_idx]
                q_ji = dec_pair_states[q_ji_idx]
                zero_pair = torch.zeros_like(q_ij)
                q_ij = torch.where(pair_valid, q_ij, zero_pair)
                q_ji = torch.where(pair_valid, q_ji, zero_pair)
                pair_rel = dec_pair_rel[q_ij_idx]
                pair_rel = torch.where(
                    pair_valid.expand(-1, pair_rel.size(1)),
                    pair_rel,
                    torch.zeros_like(pair_rel),
                )
                q_sym = self.bond_rep(torch.cat([q_ij + q_ji, torch.abs(q_ij - q_ji), q_ij * q_ji], dim=1))
                fg_sum = atom_fg_context[candidate_pairs[:, 0]] + atom_fg_context[candidate_pairs[:, 1]]
                fg_abs = torch.abs(atom_fg_context[candidate_pairs[:, 0]] - atom_fg_context[candidate_pairs[:, 1]])
                bond_block = self.bond_linear(torch.cat([q_sym, pair_rel, fg_sum, fg_abs], dim=1)).flatten()
            else:
                bond_block = torch.zeros((0,), dtype=atom_states.dtype, device=atom_states.device)

            atom_abs = torch.arange(atom_start, atom_start + atom_count, dtype=torch.long, device=atom_states.device)
            diag_pairs = torch.stack([atom_abs, atom_abs], dim=1) if atom_abs.numel() else torch.zeros((0, 2), dtype=torch.long, device=atom_states.device)
            diag_idx, diag_valid = SparsePairLayerVectorized._lookup_pair_indices(
                sparse.dec_carrier_pairs_base,
                diag_pairs,
            )
            if atom_abs.numel():
                q_diag = dec_pair_states[diag_idx]
                q_diag = torch.where(diag_valid.unsqueeze(1), q_diag, torch.zeros_like(q_diag))
                atom_block = self.atom_linear(torch.cat([atom_states[atom_abs], q_diag, atom_fg_context[atom_abs]], dim=1)).flatten()
            else:
                atom_block = torch.zeros((0,), dtype=atom_states.dtype, device=atom_states.device)
            mol_atom_states = atom_states[atom_start: atom_start + atom_count]
            graph_vec = mol_atom_states.sum(dim=0)
            if candidate_pairs.numel():
                pool_idx, pool_valid = SparsePairLayerVectorized._lookup_pair_indices(
                    sparse.dec_carrier_pairs_base,
                    candidate_pairs,
                )
                pair_values = dec_pair_states[pool_idx]
                pair_values = torch.where(pool_valid.unsqueeze(1), pair_values, torch.zeros_like(pair_values))
                pair_pool = pair_values.sum(dim=0)
            else:
    ...
    def _score_actions_reference(
        ...
        for mol_idx, (atom_start, atom_count) in enumerate(sparse.atom_scope):
            ...
            atom_block = torch.stack(atom_logits, dim=0).flatten() if atom_logits else torch.zeros((0,), dtype=atom_states.dtype, device=atom_states.device)
            mol_atom_states = atom_states[atom_start: atom_start + atom_count]
            graph_vec = mol_atom_states.sum(dim=0)
            if pair_count:
                pair_values = []
